...
export default function ProjectsItems({projectClicked, isMobile}){
    ...
    const projs = projects.map((project) => {
        return <motion.div 
            key={project.id} 
            className='flex flex-col space-y-8 justify-center items-center drop-shadow hover:drop-shadow-xl hover:scale-105 transition' 
            onClick={() => {
                projectClicked(project.name);
                    ...
                    <div className="overflow-hidden cursor-pointer relative group w-48 h-48 sm:w-46 sm:w-46 border border-zinc-700">
                        {isMobile ? <div></div> : <div className="z-50 opacity-0 group-hover:opacity-100 transition duration-500 ease-in-out cursor-pointer absolute from-black/80 to-transparent bg-gradient-to-t inset-x-0 -bottom-2 pt-30 text-white flex items-end">
                                <div>
